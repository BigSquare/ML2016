...
#data for plotting
data_X = np.arange(200000)
data_Y = [0 for y in range(200000)]
...
for comp in range(0, 1, 1):
	...
	for i in range(0, 200000, 1):
		...
		gradient_n = gradient_n + np.dot( w_wgt, 2*lamda[0]*Eta[0])
		ada_1 = np.square(gradient_n)
		ada_2 = ada_1 + ada_2
		temp_eta = np.sqrt(ada_2)
		w_wgt = w_wgt - gradient_n/temp_eta #comparing lamda and Eta, remember to change all parameters
...
wf = open('linear_regression.csv', 'w')
w = csv.writer(wf)
row_data = [['id','value']]
